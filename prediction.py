...
root='C:/Users/Acer/Documents/eye_mouse_movement-master/images'

filepaths = os.listdir(root)
X, Y = [], []
for filepath in filepaths:
  x, y, _ = filepath[6:].split(' ')  # Correctly slice the string and split
  x = float(x) / width
  y = float(y) / height
  X.append(cv2.imread(os.path.join(root, filepath)))  # Corrected file path concatenation
  Y.append([x, y])
X = np.array(X) / 255.0
Y = np.array(Y)
